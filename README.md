...
- 时间倒序画廊支持持久收藏、提示词搜索、组合筛选、批量收藏/删除/下载；删除即从索引移除，不留墓碑。进入批量模式后可直接点缩略图或文字区选图。支持按保留天数或保留张数自动清理旧图，收藏图片不参与自动清理。
- 画廊索引单独存在酒馆用户文件 `user/files/st-image-atelier-gallery.json`，每条记录只保留一份提示词；`settings.json` 里只有设置项，画廊再多也不会撑大它。升级时自动把旧数据搬过去。
